...
3. **C1 的"分布自由 1−α 覆盖集合"面临两个必须正面回答的理论缺口**:
   - **选择依赖问题**:共形推断文献 [S6] 明确边际覆盖保证在"选择后聚焦单位"上失效(selection-conditional coverage)。C1 的"真凶 ∈ 集合"恰是被挑选/领先变异的聚焦场景;若无条件覆盖论证,1−α 声明在理论上不成立。**本方法最大理论风险点**。【推论+[S6]】
...
4. **C1 的"校准阈值 q̂"有成熟程序先例**:Syring & Martin 标量校准 [S11]、bootstrap 覆盖校准 [S12]、CP4SBI conformal 校准可信集 [S13]。"校准到名义覆盖"不新;潜在新意仅剩"校准对象 = 跨多 LD × bootstrap 聚合 PIP"这一具体编排 [S11][S12]。

5. **C2 三态判定新颖性相对更高,但阈值动机缺文献支撑**。"不稳定可信集"(协议原文 [S3])、stability-guided fine-mapping [S16]、Replication Failure Rate 重采样一致性 [S15]、以及 stability selection 的"选择频率"概念 [S17](形式上与"领先集中度 c = 跨矩阵领先频率"几乎一一对应)构成概念先例;"稳健/脆弱/不可识别"命名与 0.9/0.4 阈值无直接先例——需方法方提供阈值动机或敏感性分析。
...
- 结论:主流方法不做"LD 不确定性传播",对 LD 错误的处理停留在"换更匹配的面板/敏感性模拟"。【推断】
...
- Weissbrod 等(bioRxiv 807792,PolyFun 预印本)用模拟考察 in-sample LD vs 外部参考面板的影响 [S24]。RSparsePro(bioRxiv 2024.10.29.620968)专门针对 LD mismatch 的稳健精细定位 [S26]。Benner 等(AJHG 2017)系统评估参考面板 LD vs 个体水平 GWAS LD [S21]。
...
- **Selection-conditional coverage**("Confidence on the focal: conformal prediction with selection-conditional coverage", NSF PAR purl)[S6]:边际有效的 conformal 区间在"被选择/聚焦的单元"上会失效(selection bias),需选择条件覆盖。**【C1 理论对话第一优先级对象】**
...
- **Replication Failure Rate**(Nature Genetics 2023, doi:10.1038/s41588-023-01597-3)[S15]:down-sampling 重采样一致性作为 fine-mapping 校准/可复现性指标——与 C2"跨矩阵/跨 bootstrap 一致"思路同构。【已检索】
- **stability selection**(**Meinshausen & Bühlmann 2010**, JRSS-B 72(4):417–473, doi:10.1111/j.1467-9868.2010.00740.x)[S17]:用子样本选择频率定义稳健变量,并提供有限样本错误率控制——与 C2"领先集中度 = 跨矩阵领先频率"在形式上几乎一一对应。**这是评审 C2 新颖性前必须核对的最高优先文献;本轮已确认存在与元数据,正文机制细节建议方法方自行研读。**【已检索(元数据)】
...
- 风险:0.9/0.4 阈值无文献依据;三态命名首次出现。需方法方提供阈值动机、敏感性分析,并与 stability selection [S17] / 部分识别理论对话(否则有"重新发明或有遗漏"之嫌,尤其 [S17] 形式几乎同构)。
...
1. C1 的 1−α 是逐位点条件保证还是边际保证?如何回避/论证 selection-conditional 失效 [S6]?(最高优先)
...
**[S6]** "Confidence on the focal: conformal prediction with selection-conditional coverage", NSF PAR, https://par.nsf.gov/servlets/purl/10595785 【已检索;正文未核】
